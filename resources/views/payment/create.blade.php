            ...
            <form method="post" action="{{ route('payment.store') }}">
                ...
                <div class="mb-3">
                    ...
                    <select  class="form-control form-select" id="car_id" name="car_id">
                        @forelse($cars as $car)
                            <option title="{{ Helper::carName($car) }}" value="{{ $car['car_id'] }}" {{ request()->car == $car['car_id'] ? 'selected' : null }}>{{ Helper::carName($car) }}</option>
                        @empty
                ...
                <div class="mb-3">
                    ...
                    <select  class="form-control form-select" id="type_id" name="type_id">
                        ...
                        @empty
                            <option title="Нет автомобилей" value="0">Нет категорий</option>
                        @endforelse
                    </select>
                    @error('type_id')
                        <p class="text-danger mt-2">{{ $message }}</p>
